...
from . import BRAIN_CHECKER, LOGS, bot, PLUGIN_CHANNEL_ID, CMD_HELP, LANGUAGE, MAJESTE_VERSION, PATTERNS, DEFAULT_NAME, BOT_TOKEN
...
ALIVE_MSG = [
    "        **Hey {aocsahip} **\n \n✨ Yüklenen Plugin Sayısı {plugin}\n \n👨🏼‍💻 Python Sürümü {python}\n \n⚡️Telethon Sürüm {telethon}\n \nEndişelenme UserBot'un Sapa Sağlam Çalışıyor iyi günler :)️ <3😼\n\n\n         UserBot Sürüm {aoc} ",
    "🎆 `Endişelenme! Seni yanlız bırakmam.` **{aocsahip}**, Userbot😼 çalışıyor.` \n Bot Versiyonu: {aoc} ",
    "`⛈️ Elimden gelenin en iyisini yapmaya hazırım`, **{aocsahib}** \n Bot Versiyonu: {aoc} ",
    "✨ `UserBot sahibinin emirlerine hazır...` \n Bot Versiyonu: {aoc} ",
    "`Şuan en gelişmiş userbotun düzenlediği mesajı okuyor olmalısın` **{aocsahip}**. \n Bot Versiyonu: {aoc} ",
...
CV_MSG = [
    "**{DEFAULT_NAME}** `Fazla Bi Bilgi Ayarlamamış Ama Şunu Biliyorum Kendisi Baya Zevkli Birisi Çünkü Majeste Userbot Kullanıyor.` 😁",
    "`Üzgünüm sana vercek bir bilgim yok.`"
]


UNAPPROVED_MSG = ("`Hey olduğun yerde kal,!👨‍💻 Ben` Userbot . `Endişelenme!`\n\n"
...
def extractCommands(file):
    ...
    if re.search(r'CmdHelp\(.*\)', FileRead):
        ...
        for Command in Pattern:
            ...
            # 
            Aocpy = re.search('\"\"\"AOCPY(.*)\"\"\"', FileRead, re.DOTALL)
            if not Aocpy == None:
                Aocpy = Aocpy.group(0)
                for Satir in Aocpy.splitlines():
...
try:
    bot.start()
    idim = bot.get_me().id
    aocbl = requests.get('https://raw.githubusercontent.com/HerlockBots/datas/master/blacklist.json').json()
    if idim in aocbl:
...
LOGS.info("Botunuz çalışıyor! Herhangi bir sohbete .alive yazarak Test edin."
          "Yardıma İhtiyacınız varsa, Destek grubumuza gelin t.me/majesteuserbot")
LOGS.info(f"Bot versiyonunuz: AOC ==> {AOC_VERSION}")
