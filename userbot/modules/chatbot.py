...
async def aoc(event):
    ...
    if (event.pattern_match.group(1) == "on" or event.pattern_match.group(1) == "aç" or event.pattern_match.group(1) == "ac"):
        ...
        BOT = "Y"
        await event.edit("**Aoc Yapay Zeka ChatBot Açıldı!**\n**Kullanmak için cümleye** `Aoc` **ile başlayın.**")
        ...
        BOT = "N"
        await event.edit("**Aoc Yapay Zeka ChatBot Kapandı!**")
    else:
        await event.edit("**Eksik Parametreler!** \n`.aoc chatbot` **Komutunu kullanarak talimatları okuyun.**")
...
async def txt(msg):
    ...
    if BOT == "Y":
        ...
        if message.startswith("aoc") or message.startswith("Aoc"):
            if message.startswith("aoc"):
                message = message.replace("aoc", "", 1)
            else:
                message = message.replace("Aoc", "", 1)
